...
<div class="modal-body">
    ...
    <?php if(count($result->EventAttendee) > 0) { ?>
        ...
        foreach($result->EventAttendee AS $EventAttendee) {
            ...
            $FirstName = $EventAttendee->FirstName;
            $LastName = $EventAttendee->LastName;
            if($EventAttendee->FirstName != '') {

                $badge_text = '<span class="'.$badge_or_label[0].'-'.$color[$color_id].'">'.$FirstName.' '.$LastName.'</span>&nbsp;';

                echo '<a href="'.base_url().'profile/profile">'.$badge_text.'</a>&nbsp;';
                $i++;
            }
        }
        ?>
