...
$leader_profile_id 	= real_escape_string($_POST['leader_profile_id']);
$complaint_id 		= real_escape_string($_POST['complaint_id']);
...
if($request_action == "MY_FAVOURITE_LEADER") {
	if($citizen_id == "") {
		$msg = "Sorry no citizen found";
		$error_occured = true;
	} else {

		
		$admin_view = array();
		$sel_v = "SELECT `id`, `leader_profile_id`, `fav_on` FROM `citizen_fav_leader` WHERE `citizen_id` = '".$citizen_id."' ORDER BY `fav_on` DESC";
		...
		if($num_v > 0) {
			while($res_v = fetch_array($exe_v)) {
				...
				$user_detail = return_leader_detail_limited($res_v['leader_profile_id'], get_detail_to_show_for_leader());
	...
	if($citizen_id == "") {
		...
		$search_text = real_escape_string($_POST['search_text']);
		$admin_view = array();
		
		if($search_text != '') {
			$sel_v = "SELECT `id` FROM `leader_profile` WHERE 1 = 1 
														AND (`leader_type` = '1' OR `leader_type` = '2')
														AND `status` = '1'
														AND	(`first_name` LIKE '%".$search_text."%' 
		...
		if($num_v > 0) {
			while($res_v = fetch_array($exe_v)) {
				$my_favourite = 0;
				$user_detail = return_leader_detail_limited($res_v['id'], get_detail_to_show_for_leader());
				$sel = "SELECT id FROM `citizen_fav_leader` WHERE `citizen_id` = '".$citizen_id."' AND `leader_profile_id` = '".$res_v['id']."'";
	...
	if($citizen_id == "") {
		$msg = "Please select citizen first";
		$error_occured = true;
	} else {

		if($leader_profile_id > 0) {
			$sel_v = "SELECT id FROM `citizen_fav_leader` WHERE `citizen_id` = '".$citizen_id."' AND `leader_profile_id` = '".$leader_profile_id."'";
			$exe_v = execute_query($sel_v);
			$num_v = num_rows($exe_v);
			if($num_v > 0) {
				$que = "DELETE FROM `citizen_fav_leader` WHERE `citizen_id` = '".$citizen_id."' AND `leader_profile_id` = '".$leader_profile_id."'";
				$msg = "Remove from favourite";
				$favourite = 0;
			} else {
				$que = "INSERT INTO `citizen_fav_leader` SET 
