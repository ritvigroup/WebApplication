...
if($request_action == "BLOCK_USER") {
	...
	if($user_id == "") {
		...
		$sel = "SELECT username FROM `admin` WHERE `id` != '".$user_id."' AND lower(`username`) = '".strtolower($fullname)."'";
		$exe = execute_query($sel);
		$num = num_rows($exe);
		if($num > 0) {
			...
			$sel_u = "SELECT * FROM `admin` WHERE `id` = '".$user_id."'";
	...
	if($user_id == "") {
		...
		$sel_u = "SELECT * FROM `admin` WHERE `email` = '".$email."' AND id != '".$user_id."'";
		$exe_u = execute_query($sel_u);
		$num_u = num_rows($exe_u);
		if($num_u > 0) {
