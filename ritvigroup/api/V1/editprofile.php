...
if($request_action == "BLOCK_USER") {
	...
	$city 		= real_escape_string($_POST['city']);
	$state 		= real_escape_string($_POST['state']);
	$about_me 	= real_escape_string($_POST['about_me']);
	$password 	= real_escape_string($_POST['password']);
	$date_of_birth 	= ($_POST['date_of_birth'] != '') ? $_POST['date_of_birth'] : '0000-00-00';
	$interested_in 	= ($_POST['interested_in'] > 0) ? $_POST['interested_in'] : 0;
	$search_in_meet = ($_POST['search_in_meet'] > 0) ? $_POST['search_in_meet'] : 0;
	...
	if($error_occured != true) {
		...
		if($num_u > 0) {
			...
			$upd_user = "UPDATE `users` SET 
						...
						if($interested_in != '0') {
							$upd_user .= "`interested_in` = '".$interested_in."', ";
						}
						if($search_in_meet != '0') {
							$upd_user .= "`search_in_meet` = '".$search_in_meet."', ";
						}
						if($gender != '0') {
